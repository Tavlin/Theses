Für die Bestimmung des Templates des korrelierten Untergrunds wird das Template des Signals von einer Verteilung der invarianter Masse aus der Monte Carlo Simulation abgezogen, die im gleichen $p_\text{T}$-Intervall liegt, wie das Template des Signals.
...
%Für$p_\text{T}>6.0 (\text{ GeV}/c)$ wird die Unsicherheit der Templates des korrelierten Untergrunds relativ groß im Vergleich mit der Anzahl an Einträgen in der Verteilung der invarianten Masse.
%Innerhalb der Unsicherheiten liegen die Integrale der Templates des korrelierten Untergrunds dann bei Null, wie Tabelle \ref{tab:IntAndError} im Anhang zeigt.
%Dadurch wird eine Anpassung an die Daten, unter Berücksichtigung der statistischen Unsicherheiten, erschwert.
%^geht das so^?
...
Dieses Template wird im folgenden als kombiniertes Template des korrelierten Untergrunds bezeichnet, da das vergrößerte $p_\text{T}$-Intervall mehrere der normal großen $p_\text{T}$-Intervalle kombiniert.
